...
public class PowerVNode extends AbsValueNode {
    public PowerVNode(AbsValueNode n1, AbsValueNode n2, int line) {
        super(line);
        if (n1 != null && n2 != null)
            setChildren(n1, n2);
    }
    ...
    protected Value evaluate(ExecutionContext executionContext) throws InterpretationException {
        ...
        if (valA.getType() == VariableType.NUMBER && valB.getType() == VariableType.NUMBER) {
            return new Value(NumberUtil.toNumber(Math.pow(((Number) valA.getValue()).doubleValue(),
                    ((Number) valB.getValue()).doubleValue())));
        } else
