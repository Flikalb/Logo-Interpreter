...
public class RoundVNode extends AbsValueNode {

    public RoundVNode(AbsValueNode n, int line) {
        super(line);
        if (n != null)
            setChildren(n);
    }
    ...
    protected Value evaluate(ExecutionContext executionContext) throws InterpretationException {
        ...
        if (valA.getType() == VariableType.NUMBER)
            return new Value(NumberUtil.toNumber(Math.round(((Number) valA.getValue()).doubleValue())));
        else
